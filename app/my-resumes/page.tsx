...
import { DeleteResumeForm } from "@/components/resume/DeleteResumeForm";
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 text-slate-600">Open, duplicate, or delete your saved resume versions.</p>
          </div>
        </div>
        <div className="mt-6">
          {resumes.length > 0 ? (
                ...
                return (
                  <article key={resume.id} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-2">
                      ...
                      <DeleteResumeForm resumeTitle={resume.title} action={async () => { "use server"; await deleteResume(resume.id); }} />
                    </div>
                  </article>
                );
              })}
