...
export function ElegantTwoColumn({ data, sectionOrder, isWatermarked }: ResumeTemplateProps) {
  return (
    <div className="resume-page grid grid-cols-[minmax(0,0.31fr)_minmax(0,0.69fr)] bg-[#fbfaf7] font-[Arial] text-slate-900">
      <Watermark show={isWatermarked} />
      <aside className="min-w-0 bg-[#e9e4da] px-[10mm] py-[14mm]">
        ...
        <p className="text-[10pt] font-bold uppercase tracking-[0.25em] text-stone-500">Curriculum Vitae</p>
        <h1 className="mt-4 font-[Georgia] text-[20pt] font-bold leading-[1.15] text-[#292b2d]">{data.personal.fullName}</h1>
        <p className="mt-2 text-[10pt] font-bold uppercase tracking-[0.12em] text-stone-600">{data.personal.jobTitle}</p>
        <section className="mt-7"><h2 className="mb-2 border-b border-stone-400 pb-1.5 text-[10pt] font-bold uppercase tracking-[0.16em]">Contact</h2><div className="space-y-1.5 text-[10pt] leading-[1.45] text-stone-700">{contactItems(data).map((item) => <p key={item} className="break-words">{item}</p>)}</div></section>
