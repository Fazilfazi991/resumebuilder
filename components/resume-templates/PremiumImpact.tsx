    ...
    <div className="resume-page grid grid-cols-[0.3fr_0.7fr] bg-white font-[Arial] text-[#111827]">
      ...
      <aside className="bg-[#0b1f2a] px-[8mm] py-[8.5mm] text-white">
        <div className="mb-7 flex justify-center">
          <div className="h-[43mm] w-[43mm] overflow-hidden rounded-full border-[2px] border-[#d6bd7a] bg-white/10">
            {hasText(data.personal.photoUrl) ? (
              ...
              <div className="flex h-full w-full items-center justify-center text-[28px] font-bold text-[#d6bd7a]">{initials || "RC"}</div>
            )}
          </div>
        </div>
        ...
        <SidebarBlock icon={UserRound} title="Contact">
          <div className="space-y-2.5">
            {contactItems.map(({ icon: Icon, value }) => (
              <div key={value} className="grid grid-cols-[14px_1fr] gap-3 text-[9.5px] leading-[1.35] text-white/92">
          ...
          <SidebarBlock icon={Wrench} title="Skills">
            <div className="space-y-2.5">
              {data.skills.filter((skill) => hasText(skill.name)).map((skill) => (
                <div key={skill.id}>
                  <p className="text-[9.5px] leading-[1.25] text-white">{skill.name}</p>
          ...
          <SidebarBlock icon={GraduationCap} title="Education">
            <div className="space-y-3.5">
              {data.education.filter((item) => hasText(item.degree) || hasText(item.institution)).map((item) => (
                <div key={item.id} className="avoid-break text-[9.5px] leading-[1.35]">
          ...
          <SidebarBlock icon={Languages} title="Languages">
            <div className="space-y-1.5">
              {data.languages.filter((language) => hasText(language.name)).map((language) => (
                <div key={language.id} className="grid grid-cols-[0.72fr_1fr] items-center gap-2 text-[9.5px]">
        ...
        {hasItems(data.certificates) ? (
          <SidebarBlock icon={Award} title="Certifications">
            <div className="space-y-2.5 text-[9.5px] leading-[1.4]">
      ...
      <main className="px-[12mm] py-[10mm]">
        <header className="text-center">
          <h1 className="font-[Georgia] text-[32px] font-bold uppercase leading-none tracking-[0.11em] text-[#0b1f2a]">{data.personal.fullName || "Your Name"}</h1>
          {hasText(data.personal.jobTitle) ? <p className="mt-3 text-[13px] font-bold uppercase tracking-[0.28em] text-[#b99245]">{data.personal.jobTitle}</p> : null}
            ...
            <div className="mt-5 grid grid-cols-4 divide-x divide-[#d6c7a1]">
              {stats.map(({ icon: Icon, value, label }) => (
                <div key={label} className="px-3 text-center">
                  ...
                  <p className="mt-1 text-[15px] font-bold text-[#0b1f2a]">{value}</p>
                  <p className="mt-0.5 text-[7.7px] font-bold uppercase leading-[1.25] text-[#111827]">{label}</p>
                </div>
              ))}
            </div>
          ) : null}
        ...
        {hasText(data.summary) ? (
          <MainBlock icon={UserRound} title="Professional Summary">
            <p className="text-[10.2px] leading-[1.55] text-[#111827]">{data.summary}</p>
          </MainBlock>
        ) : null}
                ...
                <TimelineItem key={item.id}>
                  <div className="flex items-start justify-between gap-5">
                    <div className="min-w-0">
                      <h3 className="text-[10.8px] font-bold text-[#0b1f2a]">{item.role}</h3>
                      <p className="mt-0.5 text-[9.5px] font-bold text-[#b99245]">{[item.company, item.location].filter(hasText).join(", ")}</p>
                    </div>
                    <p className="shrink-0 text-right text-[9px] font-semibold text-[#111827]">{dateRange(item.startDate, item.endDate, item.isCurrent)}</p>
                  </div>
                  {hasText(item.description) ? <p className="mt-1.5 text-[9.6px] leading-[1.42] text-[#111827]">{item.description}</p> : null}
                  {item.bullets.some(hasText) ? <ul className="mt-1 list-disc space-y-0.5 pl-4 text-[9.6px] leading-[1.42] text-[#111827]">{item.bullets.filter(hasText).map((bullet) => <li key={bullet}>{bullet}</li>)}</ul> : null}
                </TimelineItem>
              ))}
            </Timeline>
          </MainBlock>
              ...
              {data.projects.filter((project) => hasText(project.name)).map((project) => (
                <TimelineItem key={project.id}>
                  <div className="flex items-start justify-between gap-4">
                    <h3 className="text-[10.6px] font-bold text-[#0b1f2a]">{project.name}</h3>
                    {hasText(project.role) ? <span className="rounded-md border border-[#c8a45d] px-2 py-0.5 text-[8.5px] font-bold text-[#b99245]">{project.role}</span> : null}
                  </div>
                  {hasText(project.description) ? <p className="mt-1 text-[9.6px] leading-[1.42] text-[#111827]">{project.description}</p> : null}
                  {project.bullets.some(hasText) ? <ul className="mt-1 list-disc space-y-0.5 pl-4 text-[9.4px] leading-[1.35]">{project.bullets.filter(hasText).map((bullet) => <li key={bullet}>{bullet}</li>)}</ul> : null}
                </TimelineItem>
              ))}
              {data.achievements.filter((achievement) => hasText(achievement.title)).map((achievement) => (
                <TimelineItem key={achievement.id}>
                  <h3 className="text-[10.6px] font-bold text-[#0b1f2a]">{achievement.title}</h3>
                  {hasText(achievement.description) ? <p className="mt-1 text-[9.6px] leading-[1.42] text-[#111827]">{achievement.description}</p> : null}
                </TimelineItem>
              ))}
            </Timeline>
          </MainBlock>
        ...
        {hasItems(data.skills) ? (
          <MainBlock icon={Settings} title="Tools & Technologies">
            <div className="grid grid-cols-4 gap-2">
              {data.skills.filter((skill) => hasText(skill.name)).map((skill) => (
                <span key={skill.id} className="rounded-md border border-slate-300 px-2 py-1.5 text-center text-[8.8px] font-semibold text-[#111827]">{skill.name}</span>
              ))}
            </div>
          </MainBlock>
        ) : null}
  ...
  return (
    <section className="resume-section py-4">
      <div className="mb-3 flex items-center gap-2 border-b border-[#c8a45d] pb-1.5">
        ...
        <h2 className="text-[11px] font-bold uppercase tracking-[0.08em] text-white">{title}</h2>
      </div>
      {children}
    </section>
  );
  ...
  return (
    <section className="resume-section pt-5">
      <div className="mb-3 grid grid-cols-[27px_auto_1fr] items-center gap-2">
        ...
        <h2 className="text-[12px] font-bold uppercase tracking-[0.06em] text-[#0b1f2a]">{title}</h2>
