    ...
    <div className="resume-page grid grid-cols-[0.315fr_0.685fr] bg-white font-[Arial] text-[#0c1830]">
      ...
      <aside className="bg-[#002b3d] bg-[linear-gradient(145deg,#01283a_0%,#003d4f_48%,#012538_100%)] px-[8.5mm] py-[9mm] text-white">
        <div className="mb-7 overflow-hidden rounded-[9px] border-[2px] border-white/80 bg-white/10 shadow-sm">
          {hasText(data.personal.photoUrl) ? (
            ...
            <div className="flex h-[44mm] items-center justify-center bg-white/10 text-[22pt] font-bold tracking-normal text-teal-100">{initials || "RC"}</div>
          )}
        </div>
        ...
        <SidebarSection icon={UserRound} title="Contact">
          <div className="space-y-2.5">
            {contact.map(({ icon: Icon, value }) => (
              <div key={value} className="grid grid-cols-[14px_1fr] gap-3 text-[10pt] leading-[1.35] text-white/90">
      ...
      <main className="px-[13mm] py-[10.5mm]">
        <header>
          ...
          {hasText(data.personal.jobTitle) ? <p className="mt-3 text-[12pt] font-bold uppercase tracking-[0.18em] text-[#0b7478]">{data.personal.jobTitle}</p> : null}
          <div className="mt-5 grid grid-cols-3 gap-x-5 gap-y-3">
            {contact.map(({ icon: Icon, value }) => (
              <div key={value} className="flex items-center gap-2 text-[10pt] font-semibold text-[#26324a]">
        ...
        {hasText(data.summary) ? (
          <MainSection icon={UserRound} title="Professional Summary">
            <p className="text-[10.5pt] leading-[1.5] text-[#18233a]">{data.summary}</p>
          </MainSection>
        ) : null}
  ...
  return (
    <section className="resume-section border-b border-white/35 py-4 last:border-b-0">
      <div className="mb-3 flex items-center gap-3">
        ...
        <h2 className="font-[Georgia] text-[13pt] font-bold uppercase tracking-[0.06em] text-white">{title}</h2>
      </div>
      {children}
    </section>
  );
  ...
  return (
    <section className="resume-section border-b border-[#98a6b5] py-5 last:border-b-0">
      <div className="mb-3 flex items-center gap-3">
        ...
        <h2 className="font-[Georgia] text-[13.5pt] font-bold uppercase tracking-[0.04em] text-[#07162e]">{title}</h2>
      </div>
      {children}
    </section>
  );
...
function ProjectRow({ title, badge, description, bullets = [] }: { title: string; badge?: string; description: string; bullets?: string[] }) {
  return (
    <article className="avoid-break grid grid-cols-[34px_1fr_auto] gap-3">
      ...
      <div className="min-w-0">
        <h3 className="text-[10.5pt] font-bold text-[#0c1830]">{title}</h3>
        {hasText(description) ? <p className="mt-1 text-[10pt] leading-[1.4] text-[#26324a]">{description}</p> : null}
        {bullets.some(hasText) ? <ul className="mt-1 list-disc space-y-0.5 pl-4 text-[10pt] leading-[1.35] text-[#26324a]">{bullets.filter(hasText).map((bullet) => <li key={bullet}>{bullet}</li>)}</ul> : null}
      </div>
      {hasText(badge) ? <span className="h-fit rounded-md bg-[#e6f0f1] px-2 py-1 text-[10pt] font-bold text-[#26324a]">{badge}</span> : null}
    </article>
  );
}
