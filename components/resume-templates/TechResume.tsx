  ...
  return (
    <div className="resume-page px-[13mm] py-[12mm] font-[Arial] text-slate-900">
      ...
      <header className="bg-slate-900 px-6 py-5 text-white">
        <div className="mb-3 inline-flex border border-cyan-400/50 px-2.5 py-1 font-mono text-[8.5px] font-bold uppercase tracking-[0.18em] text-cyan-300">developer.profile</div>
        <h1 className="text-[30px] font-bold tracking-normal">{data.personal.fullName}</h1>
        <p className="mt-1 font-mono text-[12px] font-bold text-cyan-300">{data.personal.jobTitle}</p>
        <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 font-mono text-[8.75px] text-slate-300">{contactItems(data).map((item) => <span key={item}>{item}</span>)}</div>
      </header>
