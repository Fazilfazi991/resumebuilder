  ...
  return (
    <div className="resume-page grid grid-cols-[0.29fr_0.71fr] bg-[#fbfbfa] font-[Arial] text-[#111827]">
      ...
      <aside className="bg-[#4b5f48] px-[7.8mm] py-[8mm] text-white">
        ...
        <SidebarSection icon={UserRound} title="Contact">
          <div className="space-y-2.5">
            {contacts.map(({ icon: Icon, value }) => (
              <div key={value} className="grid grid-cols-[13px_1fr] gap-3 text-[9.2px] leading-[1.35] text-white/92">
            ...
            <div className="space-y-2.1">
              {data.skills.filter((skill) => hasText(skill.name)).map((skill) => (
                <div key={skill.id} className="avoid-break">
                  <p className="text-[8.8px] leading-[1.15] text-white">{skill.name}</p>
            ...
            <div className="space-y-3">
              {data.education.filter((item) => hasText(item.degree) || hasText(item.institution)).map((item) => (
                <div key={item.id} className="avoid-break border-b border-white/12 pb-2.5 last:border-b-0 last:pb-0">
                  <p className="text-[9.3px] font-bold leading-[1.25] text-white">{[item.degree, item.field].filter(hasText).join(" in ")}</p>
                  <p className="mt-1 text-[8.9px] leading-[1.3] text-white/86">{item.institution}</p>
                  <p className="mt-1 flex items-center gap-1.5 text-[8.5px] text-white/78"><CalendarDays size={9} aria-hidden="true" />{[dateRange(item.startDate, item.endDate), item.location].filter(hasText).join(" | ")}</p>
                </div>
              ))}
            </div>
          </SidebarSection>
          ...
          <SidebarSection icon={Languages} title="Languages">
            <div className="space-y-1.5">
              {data.languages.filter((language) => hasText(language.name)).map((language) => (
                <div key={language.id} className="grid grid-cols-[0.7fr_1fr] items-center gap-2 text-[9px]">
        ...
        {hasItems(data.certificates) ? (
          <SidebarSection icon={Award} title="Certifications">
            <div className="space-y-2 text-[8.9px] leading-[1.35] text-white/9">
      ...
      <main className="px-[11mm] py-[9mm]">
        <header>
          <h1 className="font-[Georgia] text-[34px] font-bold leading-none tracking-normal text-[#20262f]">{data.personal.fullName || "Your Name"}</h1>
          {hasText(data.personal.jobTitle) ? <p className="mt-3 text-[12px] font-bold uppercase tracking-[0.28em] text-[#5e6f52]">{data.personal.jobTitle}</p> : null}
          ...
          <div className="mt-5 grid grid-cols-3 gap-x-5 gap-y-3">
            {contacts.map(({ icon: Icon, value }) => (
              <div key={value} className="flex min-w-0 items-center gap-2 text-[9px] font-semibold text-[#111827]">
        ...
        {hasText(data.summary) ? (
          <MainSection icon={UserRound} title="Summary">
            <p className="text-[10.2px] leading-[1.5] text-[#111827]">{data.summary}</p>
          </MainSection>
        ) : null}
                ...
                <TimelineItem key={item.id}>
                  <div className="flex items-start justify-between gap-5">
                    <div className="min-w-0">
                      ...
                      {hasText(item.company) ? <p className="mt-1 text-[9.4px] font-bold text-[#5e6f52]">{item.company}</p> : null}
                    </div>
                  ...
                  {hasText(item.description) ? <p className="mt-1.5 text-[9.5px] leading-[1.4] text-[#111827]">{item.description}</p> : null}
              ...
              {data.projects.filter((project) => hasText(project.name)).map((project) => (
                <TimelineItem key={project.id}>
                  <div className="flex items-start justify-between gap-4">
                    <h3 className="text-[10.6px] font-bold text-[#111827]">
                      {project.name}
                      {hasText(project.link) ? <ExternalLink size={10} className="ml-1.5 inline text-[#5e6f52]" aria-hidden="true" /> : null}
                    </h3>
                    {hasText(project.role) ? <span className="rounded bg-[#5e6f52] px-2 py-1 text-[8px] font-bold text-white">{project.role}</span> : null}
                  </div>
                  {hasText(project.description) ? <p className="mt-1 text-[9.5px] leading-[1.38] text-[#111827]">{project.description}</p> : null}
                  {project.bullets.some(hasText) ? <ul className="mt-1 list-disc space-y-0.5 pl-4 text-[9.4px] leading-[1.35] text-[#111827]">{project.bullets.filter(hasText).map((bullet) => <li key={bullet}>{bullet}</li>)}</ul> : null}
                </TimelineItem>
              ))}
            </Timeline>
          </MainSection>
            ...
            <div className="grid grid-cols-3 gap-3">
              {data.achievements.filter((achievement) => hasText(achievement.title)).slice(0, 3).map((achievement, index) => (
                <article key={achievement.id} className="avoid-break rounded-md border border-[#e2e6de] bg-[#f7f8f5] p-3">
                  ...
                  <h3 className="text-[8.9px] font-bold text-[#111827]">{achievement.title}</h3>
                  {hasText(achievement.description) ? <p className="mt-1 text-[8.5px] leading-[1.35] text-[#111827]">{achievement.description}</p> : null}
                </article>
              ))}
            </div>
          </MainSection>
        ...
        {hasItems(data.skills) ? (
          <MainSection icon={Wrench} title="Tools & Technologies">
            <div className="flex flex-wrap gap-1.5">
              {data.skills.filter((skill) => hasText(skill.name)).map((skill) => (
                <span key={skill.id} className="rounded-md border border-[#d1d5db] px-2.5 py-1 text-[8.7px] font-semibold text-[#111827]">{skill.name}</span>
              ))}
            </div>
          </MainSection>
        ) : null}
  ...
  return (
    <section className="resume-section pt-5">
      <div className="mb-3 grid grid-cols-[27px_auto_1fr] items-center gap-2">
        ...
        <h2 className="text-[11.5px] font-bold uppercase tracking-[0.06em] text-[#111827]">{title}</h2>
